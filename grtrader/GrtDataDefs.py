from grtrader.GrtCoreDefs import GRTSBarStruct, GRTSOrdDtlStruct, GRTSOrdQueStruct, GRTSTickStruct, GRTSTransStruct
import numpy as np
import pandas as pd
...
class GrtNpTransactions:
    ...
    def __init__(self, forceCopy:bool = False):
        ...
        基于numpy.ndarray的逐笔成交数据容器
        @forceCopy  是否强制拷贝, 如果为True, 则会拷贝一份数据, 否则会直接引用内存中的数据
                    强制拷贝主要用于GrtDtHelper的read_dsb_trans和read_dmb_trans接口, 因为这两个接口返回的数据是临时的, 调用结束就会释放
        '''
        self.__data__:np.ndarray = None
        self.__force_copy__:bool = forceCopy
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in GrtNpTransactions")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = GRTSTransStruct*count
        if self.__force_copy__:
        ...
        npAy = np.frombuffer(c_array, dtype=self.__type__, count=count)
        # 这里有点不高效，需要拼接的地方主要是GtDtServo的场景，这里慢点没关系
        # 一旦触发拼接逻辑，都会拷贝一次
        if self.__data__ is not None:
            self.__data__ = np.concatenate((self.__data__, npAy))
            self.__data__.flags.writeable = self.__force_copy__
...
class GrtNpOrdDetails:
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in GrtNpOrdDetails")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = GRTSOrdDtlStruct*count
        if self.__force_copy__:
            c_array = DataList.from_buffer_copy(DataList.from_address(addressof(firstItem.contents)))
        else:
            c_array = DataList.from_buffer(DataList.from_address(addressof(firstItem.contents)))
...
class GrtNpOrdQueues:
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in GrtNpOrdQueues")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = GRTSOrdQueStruct*count
        if self.__force_copy__:
            c_array = DataList.from_buffer_copy(DataList.from_address(addressof(firstItem.contents)))
        else:
            c_array = DataList.from_buffer(DataList.from_address(addressof(firstItem.contents)))
        ...
        npAy = np.frombuffer(c_array, dtype=self.__type__, count=count)
        # 这里有点不高效，需要拼接的地方主要是GrtDtServo的场景，这里慢点没关系
        # 一旦触发拼接逻辑，都会拷贝一次
        if self.__data__ is not None:
            self.__data__ = np.concatenate((self.__data__, npAy))
            self.__data__.flags.writeable = self.__force_copy__
...
class GrtBarCache:
    def __init__(self, isDay:bool = False, forceCopy:bool = False):
        self.records:GrtNpKline = None
        self.__is_day__ = isDay
        self.__force_copy__ = forceCopy
        self.__total_count__ = 0

    def on_read_bar(self, firstItem:POINTER(GRTSBarStruct), count:int, isLast:bool):
        if self.records is None:
            self.records = GrtNpKline(isDay=self.__is_day__, forceCopy=self.__force_copy__)
...
class GrtTickCache:
    def __init__(self, forceCopy:bool = False):
        self.records:GrtNpTicks = None
        self.__force_copy__ = forceCopy
        self.__total_count__ = 0

    def on_read_tick(self, firstItem:POINTER(GRTSTickStruct), count:int, isLast:bool):
        if self.records is None:
            self.records = GrtNpTicks(forceCopy=self.__force_copy__)
