...
class GrtBtTask():
    ...
    回测任务类
    '''
    def __init__(self, user:str, straid:str, btid:str, folder:str, logger:GrtLogger = None, sink:BtTaskSink = None):
        self.user = user
        self.straid = straid
        self.btid = btid
        self.logger = logger
...
class GrtBtMon(BtTaskSink):
    '''
    回测管理器
    '''
    def __init__(self, deploy_folder:str, dtServo:GrtDtServo = None, logger:GrtLogger = None):
    ...
    def new_add_strategy(self, user:str, name:str) -> dict:
        if user not in self.user_stras:
            self.__load_user_data__(user)
    ...
    def run_backtest(self, user:str, code:str, straid:str, fromTime:int, endTime:int, capital:float, slippage:int=0, path:str=None) -> dict:
        ...
        # 初始化目录下的配置文件
        old_path = os.path.join(self.path, "template/configbt.yaml")
        new_path = os.path.join(folder, "configbt.yaml")
        ...
        old_path = os.path.join(self.path, "template/logcfgbt.yaml")
        new_path = os.path.join(folder, "logcfgbt.yaml")
        shutil.copyfile(old_path, new_path)
