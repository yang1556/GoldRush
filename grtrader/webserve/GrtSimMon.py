...
from grtrader.webserve import GrtBtMon, GrtLogger
...
class GrtSimMon:
    ...
    def __init__(self, dtServo: GrtDtServo = None):
        self.path = ""
    ...
    def run_as_server(self, port: int = 8081, host="127.0.0.1", bSync: bool = True):
        ...
        app = FastAPI(title="GrtSimMon", description="A simple http api of GrtSimMon", openapi_tags=tags_info,
                      redoc_url=None, version="1.0.0")
        app.add_middleware(GZipMiddleware, minimum_size=1000)
        app.add_middleware(SessionMiddleware, secret_key='!@#$%^&*()', max_age=25200, session_cookie='GrtSimMon_sid')
        app.add_middleware(
            CORSMiddleware,
            allow_origins=["*"],
    ...
    def init_bt_apis(self, app: FastAPI):
        ...
        async def cmd_del_stra(
            ...
            if self.__bt_mon__ is None:
                ...
                if not self.__bt_mon__.has_strategy("admin", straid):
                    ret = {
                        "result": -2,
                        "message": "策略不存在"
                    }
                else:
                    ret = self.__bt_mon__.del_strategy("admin", straid)
                    if ret:
                        ret = {
                            "result": 0,
                            "message": "OK"
        ...
        async def qry_bt_bars(
            ...
            # bar=dict()
            # for i in range(100):
            #     bar["bartime"] = 20170911
            #     bar["open"] = 3833.0
            #     bar["high"] = 3835.0
            #     bar["low"] = 3823.8
            #     bar["close"] = 3824.8
            #     bar["volume"] = 723.0
            #     bar["turnover"] = 2709110935
            #     bar_test.append(bar)
            # print("bar_test:",type(bar_test[0]["bartime"]))
            # print("bars:",type(bars[0]["bartime"]))
            if bars is None:
                ret = {
                    "result": -2,
