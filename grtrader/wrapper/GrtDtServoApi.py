...
from grtrader.GrtDataDefs import GrtNpKline, GrtNpTicks, GrtBarCache, GrtTickCache
from grtrader.wrapper.PlatformHelper import PlatformHelper as ph
from grtrader.GrtUtilDefs import singleton
import os

CB_GET_BAR = CFUNCTYPE(c_void_p,  POINTER(GRTSBarStruct), c_uint32, c_bool)
CB_GET_TICK = CFUNCTYPE(c_void_p,  POINTER(GRTSTickStruct), c_uint32, c_bool)
...
class GrtDtServoApi:
    ...
    def __init__(self):
        ...
        dllname = ph.getModule("GrtDtServo")
        a = (paths[:-1] + (dllname,))
    ...
    def get_bars(self, stdCode:str, period:str, fromTime:int = None, dataCount:int = None, endTime:int = 0) -> GrtNpKline:
        '''
        获取K线数据
        @stdCode    标准合约代码
        ...
        '''        
        bar_cache = GrtBarCache()
        if fromTime is not None:
            #print(stdCode,period,fromTime,endTime)
    ...
    def get_ticks(self, stdCode:str, fromTime:int = None, dataCount:int = None, endTime:int = 0) -> GrtNpTicks:
        '''
        获取tick数据
        @stdCode    标准合约代码
    ...
    def get_ticks_by_date(self, stdCode:str, iDate:int) -> GrtNpTicks:
        '''
        按天读取tick数据
        @stdCode    标准合约代码
    ...
    def get_sbars_by_date(self, stdCode:str, iSec:int, iDate:int) -> GrtNpKline:
        '''
        按天读取秒线
        @stdCode    标准合约代码
        @iSec       周期, 单位s
    ...
    def get_bars_by_date(self, stdCode:str, period:str, iDate:int) -> GrtNpKline:
        '''
        按天读取分钟线
        @stdCode    标准合约代码
        @period     周期, 分钟线
