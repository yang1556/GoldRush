from grtrader.wrapper import GrtBtWrapper
from grtrader.CtaContext import CtaContext
from grtrader.StrategyDefs import BaseCtaStrategy
from grtrader.ExtToolDefs import BaseIndexWriter
from grtrader.GrtCoreDefs import EngineType
...
class GrtBtEngine:
    ...
    def __init__(self, eType:EngineType = EngineType.ET_CTA, logCfg:str = "logcfgbt.yaml", isFile:bool = True, bDumpCfg:bool = False, outDir:str = "./outputs_bt"):
        ...
        self.__wrapper__ = GrtBtWrapper(self)  #api接口转换器
    ...
    def init_with_config(self, folder:str, 
        ...
        if type(self.__config__["replayer"]["basefiles"]["contract"]) == str:
            print(self.__config__["replayer"]["basefiles"])
            print(self.__config__["replayer"]["basefiles"]["contract"])
    ...
    def init(self, folder:str, 
        ...
        if cfgfile.lower().endswith(".json"):
            ...
            print("yaml.full_load(content): ",yaml.full_load(content))
    ...
    def configBTStorage(self, mode:str, path:str = None, storage:dict = None):
        '''
        配置数据存储
        @mode   存储模式,csv-表示从csv直接读取,一般回测使用,Grtp-表示使用Grt框架自带数据存储
        '''
